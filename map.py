...
from mpl_toolkits.basemap import Basemap  # Import Basemap for map plotting
import utility as u
...
def main():
    ...
    try:
        # Extract coordinates from stops.txt
        stop_lons = stops['stop_lon'].values
        stop_lats = stops['stop_lat'].values
    except KeyError:
        print("Error: Missing 'stop_lon' or 'stop_lat' columns in stops.txt")
        sys.exit(1)

    # Define Basemap projection with zoomed-out boundaries
    lat_padding = 0.5  # Add 0.5 degrees padding to zoom out the view
    lon_padding = 0.5

    m = Basemap(projection='merc',  # Mercator projection
                llcrnrlat=stops['stop_lat'].min() - lat_padding,  # Zoom out by decreasing the min latitude
                urcrnrlat=stops['stop_lat'].max() + lat_padding,  # Zoom out by increasing the max latitude
                llcrnrlon=stops['stop_lon'].min() - lon_padding,  # Zoom out by decreasing the min longitude
                urcrnrlon=stops['stop_lon'].max() + lon_padding,  # Zoom out by increasing the max longitude
    ...
    # Draw map details (coastlines, countries, etc.)
    m.drawcoastlines()
    m.drawcountries()
    ...
    # Convert lat/lon to the map's projection coordinates
    pos = {stop_id: m(lon, lat) for stop_id, lon, lat in zip(stops['stop_id'], stop_lons, stop_lats)}

    # Plot edges and nodes on the map
